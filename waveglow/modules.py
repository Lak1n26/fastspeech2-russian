...
class GatedDilatedConv1d(nn.Module):
    ...
    def __init__(
        ...
        if filter_width % 2 == 0:
            raise ValueError(
                "You must specify an odd number to filter_width "
        ...
        if local_condition_channels is not None:
            self.conv_lc_filter_gate = nn.Conv1d(
                local_condition_channels, dilation_channels * 2, 1, bias=False
            )
...
class WaveNet(nn.Module):
    ...
    def forward(self, sample, local_condition):
        ...
        for dilated_conv_layer in self.dilated_conv_layers:
            current_layer, skip_output = dilated_conv_layer(
                current_layer, local_condition
            )
    ...
    def __init__(self, upsample_factor, upsample_method="duplicate", squeeze_factor=8):
        ...
        if upsample_method == "duplicate":
            ...
            if not isinstance(upsample_factor, list):
                raise ValueError(
                    "You must specify upsample_factor as a list "
            ...
            raise ValueError(
                "{} upsampling is not supported".format(self._upsample_method)
            )
