...
class WaveGlowDataset(Dataset):
    ...
    def __init__(
        ...
        if metadata_file is not None:
            ...
            if self.local_condition_enabled:
                self.local_condition_files = sorted(
                    self._find_files(local_condition_dir, "*.npy")
                )
    ...
    def _load_from_metadata(self, metadata_file):
        ...
        with open(metadata_file, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(parts) != 2:
                    raise ValueError(
                        "Invalid metadata format: {}. Expected: 'path/to/mel.npy|path/to/audio.wav'".format(
                            line
                        )
                    )
                ...
                if not os.path.exists(mel_path):
                    raise ValueError(
                        "Mel-spectrogram file not found: {}".format(mel_path)
                    )
    ...
    def _check_consistency(self, audio_namelist, local_condition_namelist):
        audio_ids = [
            os.path.splitext(os.path.basename(name))[0] for name in audio_namelist
        ]
        local_condition_ids = [
...
class WaveGlowCollate(object):
    ...
    def __init__(
        ...
        self.upsample_factor = (
            int(np.prod(upsample_factor)) if upsample_factor is not None else 1
        )
        self.local_condition_enabled = local_condition_enabled
            ...
            if sample_size % self.upsample_factor != 0:
                raise ValueError(
                    ...
                    "Suggested values for upsample_factor={}: {}, {}, {}".format(
                        ...
                        self.upsample_factor
                        * (sample_size // self.upsample_factor - 1),
                        self.upsample_factor
                        * (sample_size // self.upsample_factor + 1),
                    )
                )
    ...
    def _collate_audio(self, batch):
        ...
        for idx in range(len(batch)):
            ...
            if length_diff > 0:
                sample = np.pad(sample, [[0, length_diff], [0, 0]], "constant")
            elif length_diff < 0:
        ...
        local_condition_batch = [
            np.pad(x[1], [[0, max_len - len(x[1])], [0, 0]], "edge") for x in new_batch
        ]
        ...
        local_condition_batch = torch.FloatTensor(
            np.array(local_condition_batch)
        ).transpose(1, 2)
    ...
    def _collate_audio_only(self, batch):
        ...
        sample_batch = [
            np.pad(x, [[0, max_len - len(x)], [0, 0]], "constant") for x in new_batch
        ]
